...
function Dashboard() {
  ...
  const handleAddWidget = (type) => {
    ...
    dispatch(setWidgets([...widgets, newWidget]))
    dispatch(setLayouts({
      ...layouts,
      lg: [...(layouts.lg || []), newLayout],
      md: [...(layouts.md || []), { ...newLayout, w: Math.min(size.w, 8) }],
      sm: [...(layouts.sm || []), { ...newLayout, w: Math.min(size.w, 4) }]
    }))
          ...
          <div className="flex items-center gap-3">
            ...
            {!isConfigMode && (
              <select
                ...
                onChange={(e) => dispatch({ type: 'dashboard/setDateFilter', payload: e.target.value })}
              >
                {DATE_FILTERS.map((f) => (
            ...
            <div className="flex-1">
              {widgets.length === 0 ? (
                <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center min-h-96 text-center p-8">
                  ...
                  <h3 className="text-gray-600 font-medium text-lg">Canvas is empty</h3>
                ...
                <div className="grid grid-cols-12 gap-4">
                  {widgets.map((widget) => (
                    <motion.div
                      ...
                      className={`col-span-${Math.min(widget.width, 12)} bg-white rounded-2xl border border-gray-200 p-4 relative group`}
                    >
                      {/* Widget Header */}
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="text-sm font-medium text-gray-800">{widget.title}</p>
                          <p className="text-xs text-gray-400 capitalize">{widget.type.replace('-', ' ')}</p>
                        </div>
                        {/* Hover Actions */}
          ...
          <div>
            {widgets.length === 0 ? (
              <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center min-h-96 text-center p-8">
                ...
                <h3 className="text-gray-600 font-medium text-lg">No widgets configured</h3>
              ...
              <div className="grid grid-cols-12 gap-4">
                {widgets.map((widget) => (
                  <motion.div
                    ...
                    className={`col-span-${Math.min(widget.width, 12)} bg-white rounded-2xl border border-gray-200 p-4`}
                  >
                    <p className="text-sm font-medium text-gray-800 mb-1">{widget.title}</p>
                    <p className="text-xs text-gray-400 capitalize mb-3">{widget.type.replace('-', ' ')}</p>
                    <div className="h-32 bg-gray-50 rounded-xl flex items-center justify-center">
                      <p className="text-gray-400 text-xs capitalize">
                        {widget.type.replace('-', ' ')} coming soon
                      </p>
                    </div>
                  </motion.div>
                ))}
