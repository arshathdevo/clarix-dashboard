...
const generateResponsiveLayouts = (lgLayout) => {
  if (!lgLayout || !lgLayout.length) return { lg: [], md: [], sm: [], xs: [], xxs: [] }
  let mdY = 0, smY = 0, xsY = 0
  return {
    lg: lgLayout,
    md: lgLayout.map((item) => {
      const w = Math.min(item.w, 8)
      const layout = { ...item, w, x: 0, y: mdY }
      mdY += item.h
      return layout
    }),
    sm: lgLayout.map((item) => {
      const layout = { ...item, w: 4, x: 0, y: smY }
      smY += item.h
      return layout
    }),
    xs: lgLayout.map((item) => {
      const layout = { ...item, w: 4, x: 0, y: xsY }
      xsY += item.h
      return layout
    }),
    xxs: lgLayout.map((item, i) => ({
      ...item, w: 4, x: 0, y: i * item.h
    }))
  }
}
...
function Dashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const savedLayouts = res.data.layouts || {}
      const fetchedWidgets = res.data.widgets || []
      const lgLayout = savedLayouts.lg || []
      const generated = generateResponsiveLayouts(lgLayout)
      const finalLayouts = {
        lg: lgLayout,
        md: savedLayouts.md?.length ? savedLayouts.md : generated.md,
        sm: savedLayouts.sm?.length ? savedLayouts.sm : generated.sm,
        xs: generated.xs,
        xxs: generated.xxs
      }
      dispatch(setLayouts(finalLayouts))
      dispatch(setWidgets(fetchedWidgets))
    } catch (error) {
  ...
  const handleAddWidget = (type) => {
    ...
    const maxY = existingLg.reduce((max, l) => Math.max(max, l.y + l.h), 0)
    const newLayoutItem = { i: id, x: 0, y: maxY, w: size.w, h: size.h, minW: 1, minH: 2 }
    const mobileItem = { ...newLayoutItem, w: 4, x: 0 }

    dispatch(addWidget(newWidget))
    dispatch(setLayouts({
      lg: [...(layouts.lg || []), newLayoutItem],
      md: [...(layouts.md || []), { ...newLayoutItem, w: Math.min(size.w, 8) }],
      sm: [...(layouts.sm || []), mobileItem],
      xs: [...(layouts.xs || []), mobileItem],
      xxs: [...(layouts.xxs || []), mobileItem]
    }))
  ...
  const handleDeleteWidget = (id) => {
    ...
    dispatch(setLayouts({
      ...
      xs: (layouts.xs || []).filter((l) => l.i !== id),
      xxs: (layouts.xxs || []).filter((l) => l.i !== id)
    }))
  ...
  const gridProps = {
    breakpoints: { lg: 1200, md: 768, sm: 480, xs: 375, xxs: 0 },
          ...
          {isConfigMode ? (
            ...
            >
              ...
              <motion.div
                ...
                className="w-full lg:w-60 lg:flex-shrink-0"
              >
                <div className="bg-white rounded-2xl border border-gray-200 p-4 lg:sticky lg:top-4 shadow-sm">
                          ...
                          <div className="drag-handle flex items-center justify-between px-4 pt-3 pb-2 flex-shrink-0 cursor-grab active:cursor-grabbing select-none">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-400 capitalize">{widget.type.replace(/-/g, ' ')}</p>
                              </div>
                            </div>
                ...
                <div className="bg-gray-50 rounded-2xl border border-gray-200 p-1">
                  ...
                  >
                    {widgets.map((widget) => (
                      <div
                        key={widget.id}
                        className="bg-white rounded-2xl border border-gray-100 flex flex-col shadow-sm hover:shadow-md transition-shadow overflow-hidden relative"
                      >
                        <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${WIDGET_GRADIENT[widget.type]}`} />
