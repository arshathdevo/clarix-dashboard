...
function Dashboard() {
  ...
  const generateResponsiveLayouts = (lgLayout) => {
    if (!lgLayout || !lgLayout.length) return {}
    let mdY = 0
    let smY = 0
    return {
      lg: lgLayout,
  ...
  const fetchDashboard = async () => {
    try {
      ...
      // Auto generate md and sm layouts if missing
      const lgLayout = savedLayouts.lg || []
      const finalLayouts = {
        lg: lgLayout,
        md: savedLayouts.md?.length ? savedLayouts.md : generateResponsiveLayouts(lgLayout).md,
        sm: savedLayouts.sm?.length ? savedLayouts.sm : generateResponsiveLayouts(lgLayout).sm
      }
            ...
            >
              ...
              >
                <div className="bg-white rounded-2xl border border-gray-200 p-4 lg:sticky lg:top-4 shadow-sm">
                  ...
                  <div className="space-y-2 lg:space-y-2 flex flex-wrap lg:flex-nowrap lg:flex-col gap-2 lg:gap-0">
                    {groups.map((group) => (
                      <div key={group}>
                        <button
